...
# Y que NO corte el turno: la accion estuvo bien.
if awk '/GENERALIZADO EL 2026-08-14/,/^  fi$/' "$A" | grep -q 'LOOP_DETECTADO=1'; then
  _mal "no corta el turno" "cortar castiga una accion correcta y deja al usuario sin nada"
else
  _ok "no corta el turno: solo empuja a avanzar"
fi
...
if [ "$(wc -l < "$BLOQUE")" -lt 15 ]; then
  ...
  case "$r" in AVISO*) _ok  "el caso real de seis escrituras queda atrapado" ;;
               *)      _mal "el caso real" "obtuvo: $r" ;; esac
