...
// TAREAS DELEGADAS (2026-08-08). Una tarea larga corre por fuera de la conversación y puede
// terminar veinte minutos después, cuando el turno que la lanzó ya no existe. Por eso no puede
// avisar por el stderr del proceso -- como hacen las aprobaciones -- y necesita que alguien mire.
//
// A DIFERENCIA de la carpeta de aprobaciones, ESTA NO SE BORRA AL ARRANCAR: el sentido de delegar
// es que la tarea sobreviva a que se cierre Mentis. Si se limpiara al abrir, cerrar la aplicación
// perdería justo lo que se mandó a hacer aparte.
//
// El aviso reusa notifyIfUnfocused, que ya existía para las generaciones largas: sólo molesta si
// el usuario está en otra ventana, porque si está mirando Mentis ya lo ve solo.
const DELEGACIONES_DIR = path.join(MENTIS_ENV_DIR, 'delegaciones');
try {
  fsSync.mkdirSync(DELEGACIONES_DIR, { recursive: true });
  process.env.MENTIS_DELEGACIONES_DIR = DELEGACIONES_DIR;
} catch { /* sin carpeta, delegar sigue andando: lo que se pierde es el aviso */ }

// Se vigila por sondeo y no con fs.watch a propósito: fs.watch en Windows sobre carpetas de red
// o unidades montadas falla en silencio y no vuelve a avisar nunca. Cinco segundos es de sobra
// para algo que tarda minutos, y el costo es leer una carpeta casi siempre vacía.
const delegacionesAvisadas = new Set();
function revisarDelegacionesTerminadas() {
  let avisos;
  try { avisos = fsSync.readdirSync(DELEGACIONES_DIR).filter((f) => f.endsWith('.aviso')); }
  catch { return; }
  for (const archivo of avisos) {
    const id = archivo.replace(/\.aviso$/, '');
    if (delegacionesAvisadas.has(id)) continue;
    delegacionesAvisadas.add(id);
    let estado = 'terminada';
    let tarea = '';
    try { estado = fsSync.readFileSync(path.join(DELEGACIONES_DIR, archivo), 'utf-8').trim() || estado; } catch {}
    try {
      const d = JSON.parse(fsSync.readFileSync(path.join(DELEGACIONES_DIR, `${id}.json`), 'utf-8'));
      tarea = String(d.tarea || '').slice(0, 120);
    } catch {}
    // El texto dice QUÉ pasó, no sólo "listo": una tarea que falló y una que terminó bien no
    // pueden avisar igual, o el aviso deja de significar algo.
    const titulos = {
      terminada: 'Terminé la tarea que me diste aparte',
      sin_terminar: 'La tarea aparte quedó sin terminar',
      fallo: 'La tarea aparte falló',
      cancelada: 'Tarea aparte cancelada'
    };
    notifyIfUnfocused(titulos[estado] || titulos.terminada, tarea || id);
    sendToRenderer('mentis:log', `[mentis] tarea delegada ${id}: ${estado}`);
    try { fsSync.unlinkSync(path.join(DELEGACIONES_DIR, archivo)); } catch {}
  }
}
setInterval(revisarDelegacionesTerminadas, 5000);
...
function startProcessForConversation(id, flags) {
  ...
  // Respuesta final llegando por chunks (2026-08-06). nv_stream.py la emite des-escapada como
  // "NVANSWER <trozo>", con los saltos de linea como \n literal para que cada trozo sea UNA linea
  // (si no, un salto partiria el trozo en dos eventos y el panel de progreso se comeria la mitad).
  const ANSWER_MARKER = /^NVANSWER (.*)$/;
  proc.on('log', (line) => {
    const ans = ANSWER_MARKER.exec(line);
    if (ans) {
      sendToRenderer('mentis:answer-chunk', ans[1].replace(/\\n/g, '\n'));
      return;   // no va al log: es la respuesta, no progreso
    }
...
// pesado NO vive aca: vive en mentis-publicar.sh, que tiene todos los frenos (tests en verde,
// notas escritas, version mayor, firma). Esto solo lo invoca y devuelve lo que dijo.
//
// Duplicar esos frenos en la interfaz seria garantizar que un dia digan cosas distintas, y que
// alguien publique desde la app algo que el script habria rechazado.
ipcMain.handle('mentis:es-administrador', () => settingsStore.esAdministrador(MENTIS_ENV_DIR));
ipcMain.handle('mentis:estado-publicacion', () => settingsStore.getEstadoPublicacion(MENTIS_ENV_DIR));

const PUBLICAR_SCRIPT = path.join(MENTIS_ENV_DIR, 'mentis-publicar.sh');
function correrPublicar(args, timeoutMs) {
  return new Promise((resolve) => {
    if (!settingsStore.esAdministrador(MENTIS_ENV_DIR)) {
      // Segundo cerrojo: aunque alguien active el switch a la fuerza en la interfaz, sin la clave
      // privada esto no corre. Y aunque corriera, sin firma las otras Mentis lo rechazan.
      return resolve({ ok: false, salida: 'Esta maquina no tiene la clave de administrador.' });
    }
    execFile(BASH_PATH, [PUBLICAR_SCRIPT,...args],
      { cwd: MENTIS_ENV_DIR, timeout: timeoutMs || 600000, maxBuffer: 8 * 1024 * 1024 },
      (err, stdout, stderr) => {
        resolve({ ok: !err, salida: String(stdout || '') + String(stderr || '') });
      });
  });
}
ipcMain.handle('mentis:publicar-revisar', () => correrPublicar(['revisar'], 120000));
// El timeout largo es a proposito: 'publicar' corre las suites de tests antes de dejar publicar,
// y eso tarda minutos. Cortarlo antes seria publicar sin haberlos corrido.
ipcMain.handle('mentis:publicar', (_event, notas) => correrPublicar(['publicar', String(notas || '')], 900000));
ipcMain.handle('mentis:publicar-retirar', () => correrPublicar(['retirar'], 60000));
// OJO: aca habia un SEGUNDO ipcMain.handle('mentis:save-apariencia'), identico al de la linea 661.
// Electron no lo permite: tira "Attempted to register a second handler" como excepcion NO
// capturada mientras carga main.js, y lo que viene despues de esa linea no se registra nunca.
// Sin empaquetar no se notaba, porque el error solo aparece al cargar el main del paquete.
// Se saco el duplicado (2026-08-08); el bueno es el de arriba, agrupado con save-voz/save-profile.
